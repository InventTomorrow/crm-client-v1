...
export function OnboardingShell({ currentStep, children, wide = false }: Props) {
  ...
  return (
    <>
      {/* Fixed tabs — pinned below the logo header regardless of scroll. */}
      <div className="fixed top-16 inset-x-0 z-40 bg-[var(--bg)]/95 backdrop-blur-sm border-b border-[var(--line)]">
        <div className={cn('w-full mx-auto px-4 py-4', wide ? 'max-w-[900px]' : 'max-w-[520px]')}>
      ...
      {/* Centered content, offset below the fixed header + tabs. */}
      <motion.div
        key={currentStep}
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.35, ease: 'easeOut' }}
        className={cn('w-full mx-auto pt-20', wide ? 'max-w-[900px]' : 'max-w-[520px]')}
      >
        <motion.div
