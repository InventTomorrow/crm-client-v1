...
export function usePractitionerForm(practitionerId?: string) {
  ...
  // One section at a time; Radix sends '' when the open one is collapsed.
  const [openSectionId, setOpenSectionId] = useState<PractitionerSectionId | ''>(
    FIRST_SECTION,
  );
  ...
  const handleSubmit = form.handleSubmit(
    ...
    (errors) => {
      ...
      // nothing on screen to explain it, so the first failing section opens.
      const [firstFailingSection] = sectionsWithErrors(Object.keys(errors));
      if (firstFailingSection) setOpenSectionId(firstFailingSection);
    },
  );
  ...
  return {
    ...
    handleSubmit,
    openSectionId,
    // Radix hands back a plain string; the ids are a closed set, so narrowing
    // here keeps the section type intact everywhere else.
    setOpenSectionId: (next: string) =>
      setOpenSectionId(next as PractitionerSectionId | ''),
    summaries,
    erroredSections,
  };
}
