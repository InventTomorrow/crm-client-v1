...
export function WorkspaceTourStarter() {
  ...
  const isWelcomeDialogOpen = useWelcomeDialogStore((state) => state.isWelcomeDialogOpen);
  const hasStartedRef = useRef(false);

  const tourId = tourIdFor(tenant?.businessVertical);
  const hasSeenTour = !tourId || (user?.completedTours?.includes(tourId) ?? true);
  ...
  useEffect(() => {
    ...
    if (isWelcomeDialogOpen || localStorage.getItem(SHOW_DEMO_FLAG) === '1') return;
