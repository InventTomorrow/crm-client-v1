...
import { z } from 'zod';
import { Plus, Upload, X, Check, Loader2, ImageIcon } from 'lucide-react';
import { toast } from 'sonner';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/shared/ui/Dialog';
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/shared/ui/form';
import { ImageUploader } from '@/shared/ui/ImageUploader';
import { pkr, extractErrorMessage } from '@/lib/utils';
import { productSchema, CATEGORIES } from '../types';
...
const EMPTY_ITEM = (): BulkItem => ({
  name: '', sku: '', price: 0, stock: 0, cat: 'Apparel', desc: '', imageUrl: '',
});
...
function EditPanel({
...
}: { item: BulkItem; onSave: (p: BulkItem) => void; onDelete: () => void }) {
  ...
  const form = useForm<z.input<typeof productSchema>, any, ProductFormData>({
    ...
    defaultValues: {
      ...
      stock: item.stock, cat: item.cat, desc: item.desc ?? '',
    },
  });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-2.5 h-full">
        ...
        {uploading ? (
          ...
          <ImageUploader value={imageUrl || null} onChange={v => setImageUrl(v ?? '')} onUpload={handleUpload} compact />
        )}

        <FormField control={form.control} name="name" render={({ field }) => (
          <FormItem>
            <FormLabel className="text-[11.5px]">Name *</FormLabel>
            <FormControl><input className="input text-[12.5px] py-1.5" placeholder="Product name" {...field} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
        ...
        <div className="grid grid-cols-2 gap-2">
          <FormField control={form.control} name="sku" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><input className="input text-[12.5px] py-1.5" placeholder="SKU" {...field} /></FormControl>
            </FormItem>
          )} />
        ...
        <div className="grid grid-cols-2 gap-2">
          <FormField control={form.control} name="price" render={({ field }) => (
            <FormItem>
              ...
              <FormControl><input className="input text-[12.5px] py-1.5" type="number" {...field} /></FormControl>
              <FormMessage />
            </FormItem>
          )} />
        ...
        <FormField control={form.control} name="desc" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><textarea className="input text-[12.5px]" rows={2} {...field} /></FormControl>
          </FormItem>
        )} />
        ...
        <div className="flex gap-2 mt-auto pt-2 border-t border-[var(--line)]">
          ...
          <button type="submit" className="btn btn-grad text-[12px] flex-1 justify-center">
            <Check size={12} /> Apply
          </button>
        </div>
      </form>
...
function BulkCard({
...
}: { item: BulkItem; index: number; active: boolean; onClick: () => void; onRemove: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-[12px] font-medium truncate">{item.name || `Item ${index + 1}`}</div>
      <div className="flex items-center justify-between text-[11px]">
        <span className="font-[var(--font-mono)] text-[var(--ink-soft)]">{item.price ? pkr(item.price) : '—'}</span>
        <span className={`badge py-px px-1.5 font-medium ${item.stock > 0 ? 'bg-[rgba(34,197,94,0.12)] text-[#15803D]' : 'bg-[rgba(239,68,68,0.12)] text-[#DC2626]'}`}>
          {item.stock}
        </span>
      </div>
      {bad && <div className="absolute top-1.5 right-4 px-1 py-px rounded-full text-[9px] font-bold text-white bg-[#EF4444]">!</div>}
      <button
        type="button"
...
// ── Main BulkAddDialog ─────────────────────────────────────
export function BulkAddDialog({ open, onClose, onSaveAll, isSaving = false }: {
  open: boolean;
  onClose: () => void;
  ...
  const parseFile = (file: File) => {
    ...
    reader.onload = e => {
      ...
      try {
        ...
        addItems(parsed);
        toast.success(`${parsed.length} product${parsed.length !== 1 ? 's' : ''} imported`);
      } catch (err) {
  ...
  const valid = items.length > 0 && items.every(p => p.name && p.price > 0 && p.stock >= 0);
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v && !isSaving) onClose(); }}>
      <DialogContent className="flex flex-col gap-0 p-0 sm:max-w-[900px] h-[min(680px,92vh)] overflow-hidden" showCloseButton={false}>
        {/* Header */}
        <DialogHeader className="flex-shrink-0 flex-row items-start justify-between gap-2 px-5 py-3.5 border-b border-[var(--line)]">
          <div>
            <DialogTitle className="text-[16px] font-semibold">Bulk add products</DialogTitle>
            <DialogDescription className="text-[11.5px] mt-0.5 text-[var(--ink-mute)]">
              Import CSV / JSON or add manually. Select a card to edit details and upload images.
            </DialogDescription>
          </div>
        ...
        <div className="px-5 pt-3 flex-shrink-0">
          ...
          >
            ...
            <div className="flex-1 text-[12.5px]">
              ...
              <span className="text-[var(--ink-mute)]"> · columns: <code>name, sku, price, stock, category, description, image_url</code></span>
            </div>
            <button className="btn btn-outline text-[12px]" onClick={e => { e.stopPropagation(); fileRef.current?.click(); }}>
              Browse
            </button>
            <input ref={fileRef} type="file" accept=".csv,.json" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) parseFile(f); e.target.value = ''; }} />
          </div>
        </div>
          ...
          <div className="flex flex-col flex-1 min-w-0 border-r border-[var(--line)] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-2 border-b border-[var(--line)]">
              <span className="text-[12px] text-[var(--ink-soft)]">
                {items.length === 0 ? 'No products yet' : `${items.length} product${items.length > 1 ? 's' : ''}`}
              </span>
              <button
            ...
            <div className="scroll flex-1 overflow-y-auto p-3">
              <div className="grid grid-cols-[repeat(auto-fill,minmax(130px,1fr))] gap-2">
                ...
                <button
                  onClick={() => { const idx = items.length; setItems(prev => [...prev, EMPTY_ITEM()]); setSelectedIdx(idx); }}
          ...
          <div className="w-[300px] flex-shrink-0 flex flex-col overflow-hidden">
            {selectedItem !== null && selectedIdx !== null ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center gap-2 text-[var(--ink-mute)] p-6 text-center">
                ...
                <span className="text-[12.5px]">Select a product card to edit it</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-[var(--line)] flex-shrink-0">
          <span className={`text-[12px] flex items-center gap-1.5 ${valid ? 'text-[var(--ink-soft)]' : 'text-[#B45309]'}`}>
            {isSaving
              ? <><Loader2 size={12} className="animate-spin text-[var(--accent)]" /> Saving {items.length} product{items.length === 1 ? '' : 's'}…</>
              : items.length > 0
                ? valid
          ...
          <div className="flex gap-2">
            ...
            <button className="btn btn-grad" disabled={!valid || isSaving} onClick={() => onSaveAll(items)}>
              {isSaving ? (
                <><span className="animate-spin inline-block h-3.5 w-3.5 border-2 border-current border-t-transparent rounded-full" /> Saving…</>
              ) : (
                <><Check size={13} /> Save {items.length || ''} product{items.length === 1 ? '' : 's'}</>
              )}
            </button>
          </div>
        </div>
