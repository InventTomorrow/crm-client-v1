...
export default function LeadCard({
  ...
  return (
    ...
    >
      ...
      {lead.lastMsg ? (
        <div className="text-[11.5px] truncate text-[var(--ink-soft)] leading-snug">
          {lead.lastMsg}
        </div>
      ) : null}

      <div className="flex items-center justify-between gap-1.5 mt-0.5">
        <span className="flex items-center gap-1 text-[10.5px] text-[#25D366]">
          <WhatsAppIcon size={11} />
          <span>WhatsApp</span>
        </span>
        <span className="text-[10.5px] text-[var(--ink-mute)]">{lead.time}</span>
      </div>
    </div>
  );
}
