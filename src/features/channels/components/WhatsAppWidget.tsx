...
/**
 * WhatsApp channel card for the Channels page. Renders the same QR-scan flow as
 * the header dialog inline (via WAConnectPanel), so users can link a device
 * without opening a modal. Connect/disconnect are gated by `channels:connect`.
 */
export function WhatsAppWidget({
  className,
  onClose,
}: {
  /** Extra classes for the root card (e.g. to drop the border inside a dialog). */
  className?: string;
  /** When set, shows a Close button in the connected state (dialog usage). */
  onClose?: () => void;
} = {}) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="flex items-center gap-3 border-b border-[var(--line)] p-4"
        style={
          accent
            ? {
                background: `radial-gradient(120% 140% at 0% 0%, ${accent}1f 0%, transparent 60%), var(--surface)`,
              }
            : undefined
        }
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[11.5px] text-[var(--ink-mute)]">
            {isConnected && phone ? `+${phone}` : "Link your number"}
          </div>
        </div>
        <span
          className={cn(
