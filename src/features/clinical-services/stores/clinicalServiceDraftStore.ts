...
interface ClinicalServiceDraftState {
...
/**
 * The hand-off between "Add service" and the record it creates.
 *
 ...
 * client-side navigation. Nothing else lives here: the service itself is always
 * read back from the server, and the position is ignored unless `draftId`
 * matches the record being opened, which is what makes "Add service" start clean.
 *
 ...
 */
export const useClinicalServiceDraftStore = create<ClinicalServiceDraftState>()((set) => ({
  ...NO_DRAFT,
