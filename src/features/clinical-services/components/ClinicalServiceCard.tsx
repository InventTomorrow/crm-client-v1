...
export function ClinicalServiceCard({
  ...
  return (
    <article
      className={cn(
        "group flex h-full flex-col overflow-hidden rounded-[var(--radius-card)] border border-[var(--accent)]/30 bg-[var(--surface)] shadow-[var(--shadow-1)] transition-[border-color,box-shadow] hover:border-[var(--accent)]/60 hover:shadow-[var(--shadow-2)]",
        // A retired service stays readable but visibly recedes from the live ones.
        !service.isActive && "opacity-70",
      )}
    >
      {/* Tinted band so the name reads as the card's title, not the first line of copy. */}
      <header className="flex items-start justify-between gap-3 border-b border-[var(--accent)]/30 bg-[var(--accent-soft)] px-4 py-3">
        ...
        <span className="shrink-0 rounded-full border border-[var(--accent)]/30 bg-[var(--surface)] px-2 py-0.5 text-[11px] font-medium text-[var(--accent)]">
