...
export const bookingConfigFormSchema = z
  .object({
    ...
    workingHours: z.array(bookingWindowSchema).min(1, 'Add at least one time window'),
  ...
  .refine(
    (config) =>
      config.workingHours.some(
        (window) => minutesBetween(window.startTime, window.endTime) >= config.durationMinutes,
      ),
    {
...
export type QuickAppointmentFormData = z.infer<typeof quickAppointmentFormSchema>;
export type QuickAppointmentFormInput = z.input<typeof quickAppointmentFormSchema>;
...
export const manualBookingFormSchema = z
  ...
  .refine(
    (booking) => !booking.isCustomTime || new Date(booking.scheduledAt).getTime() > Date.now(),
...
export type AppointmentStatusCounts = Partial<Record<AppointmentStatus, number>>;
