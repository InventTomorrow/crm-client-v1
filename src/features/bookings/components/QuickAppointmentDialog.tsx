...
export function QuickAppointmentDialog({
  ...
  const { form, isBooking, handleSubmit, selectLead, clearLead } = useQuickAppointmentForm({
    isOpen: open,
    timezone,
    initialScheduledAt,
    ...
    >
      <DialogContent className="flex max-h-[92vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-[520px]">
        <DialogHeader className="px-5 py-4">
          <DialogTitle className="text-[15.5px] font-semibold">Add an appointment</DialogTitle>
          <DialogDescription className="text-[12.5px] leading-relaxed text-[var(--ink-mute)]">
            Books the exact time you enter, in {timezone.replace('_', ' ')} — your published
          ...
          <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
                      ...
                      <Popover open={isDatePickerOpen} onOpenChange={setIsDatePickerOpen}>
                        <PopoverTrigger asChild>
                          <FormControl>
                            ...
                            >
                              <CalendarDays size={14} className="mr-2 text-[var(--ink-mute)]" />
                              {field.value
                                ? fromDateKey(field.value)?.toLocaleDateString('en-GB', {
                                    weekday: 'short',
                                    day: 'numeric',
                                    month: 'short',
                                    year: 'numeric',
            ...
            <div className="flex shrink-0 items-center justify-end gap-2 border-t border-[var(--line)] px-5 py-3.5">
              ...
              <Button type="submit" disabled={isBooking}>
                {isBooking && <Loader2 size={14} className="mr-1.5 animate-spin" />}
