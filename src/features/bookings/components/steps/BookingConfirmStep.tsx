...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2">
      <span className="shrink-0 text-[12px] text-[var(--ink-mute)]">{label}</span>
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-2)] px-4 py-2">
        <SummaryRow label="Lead" value={booking.customerName || 'Unnamed lead'} />
      ...
      {booking.isCustomTime && (
        <p className="rounded-lg border border-dashed border-[var(--line)] px-3 py-2.5 text-[11.5px] text-[var(--ink-mute)]">
          This time sits outside your configured hours, so the bot would never have offered
          it. It is still checked against everything already booked.
        </p>
      )}
      ...
      <div className="flex flex-col gap-2">
        ...
        {answeredQuestions.length === 0 ? (
          <p className="text-[11.5px] text-[var(--ink-mute)]">
            None — the lead keeps whatever score they already had.
          </p>
        ) : (
          <ul className="flex flex-col gap-1">
            {answeredQuestions.map((question) => (
              <li key={question.fieldName} className="text-[11.5px] text-[var(--ink-mute)]">
                <span className="text-[var(--ink)]">{question.questionText}</span>{' '}
