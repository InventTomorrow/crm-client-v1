...
export function RescheduleAppointmentDialog({
  ...
  // Scoped to this appointment's own calendar: offering the clinic's shared free time
  // for a booking that belongs to one doctor would propose slots they cannot take.
        ...
        <DialogHeader className="flex-row items-start gap-3 px-5 py-4">
          ...
          <div className="min-w-0">
            <DialogTitle className="text-[15.5px] font-semibold">
              Reschedule this {vocabulary.bookingSingular}
            </DialogTitle>
            <DialogDescription className="mt-1 text-[12.5px] leading-relaxed text-[var(--ink-mute)]">
              Currently{" "}
              {formatAppointmentSlot(appointment.scheduledAt, timezone)} with{" "}
              {appointment.customerName ??
                appointment.lead?.name ??
                `this ${vocabulary.customerSingular}`}
              . Picking a new time frees the old slot and messages them on
              WhatsApp.
            </DialogDescription>
          </div>
        </DialogHeader>
