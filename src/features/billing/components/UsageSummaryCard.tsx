...
function fmtResetDate(value: string | null): string | null {
  ...
  return new Date(value).toLocaleDateString('en-PK', { month: 'short', day: 'numeric' });
}
...
export function UsageSummaryCard() {
        ...
        <div className="animate-pulse space-y-3">
          ...
          <div className="grid gap-x-5 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-9 rounded-md bg-[var(--line)] dark:bg-white/10" />
            ))}
          </div>
        </div>
      </div>
  ...
  const nearingLimit = usage.metrics.some((m) => m.limit > 0 && m.used / m.limit >= WARN_AT);
    ...
    <div className="card p-[18px]">
      ...
      <div className="grid gap-x-5 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
        {usage.metrics.map((metric) => {
          ...
          const warning =
            !exhausted && metric.limit > 0 && metric.used / metric.limit >= WARN_AT;
          return (
            <div key={metric.key}>
              <div className="flex items-baseline justify-between gap-2">
                <span className="text-[12px] text-[var(--ink-soft)]">{metric.label}</span>
                <span
                ...
                >
                  {metric.used.toLocaleString('en-PK')} / {metric.limit.toLocaleString('en-PK')}
                </span>
              </div>
              <Progress
