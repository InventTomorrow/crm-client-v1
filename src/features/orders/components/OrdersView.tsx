...
import { ORDER_STATUSES, type Order, type OrderListItem, type OrderFilters, type OrderStatus } from '../types';
import { formatMoney, ORDER_STATUS_META } from '../lib/format';
...
export function OrdersView() {
  ...
  const openEdit = (order: Order) => { setSelectedId(null); setEditing(order); setFormOpen(true); };
  ...
  const columns: ColumnDef<OrderListItem, unknown>[] = useMemo(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="min-w-0">
            ...
            {o.lead?.phone && (
              <div className="text-[11.5px] text-[var(--ink-mute)]">{o.lead.phone}</div>
            )}
          </div>
        );
      },
    },
    {
      ...
      size: 70,
      cell: ({ row }) => (
        <span className="text-[13px] text-[var(--ink-soft)]">{row.original.items.length}</span>
      ),
    },
    {
      id: 'total',
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl">
        ...
        toolbar={
          <div className="flex items-center gap-2 flex-1">
            <div className="relative flex-1 max-w-[340px]">
              <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[var(--ink-mute)]" />
              <input
      ...
      {selectedId && (
        <OrderDetailSheet orderId={selectedId} onClose={() => setSelectedId(null)} onEdit={openEdit} />
      )}
